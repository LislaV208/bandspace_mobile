...
- ZAWSZE używaj ErrorDialog z lib/shared/widgets/dialogs/error_dialog.dart do wyświetlania błędów użytkownikowi - przekazuj cały obiekt błędu (NIE używaj getErrorMessage() w cubitach), ErrorDialog automatycznie ekstraktuje odpowiedni komunikat. Format: `ErrorDialog.show(context, error: e, onRetry: () => cubit.retryOperation());`. NIE przechowuj komunikatów błędów w stanach cubitów jeśli błąd jest pokazywany w dialogu - centralizacja logiki ekstrakcji błędów w ErrorDialog eliminuje duplikację i zapewnia spójny UX. Używaj getErrorMessage() TYLKO gdy błąd musi być pokazany inline w UI (nie w dialogu) poprzez stan cubita