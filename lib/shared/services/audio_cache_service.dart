...
import 'package:bandspace_mobile/shared/services/audio_cache_database.dart';
...
class AudioCacheService {
  ...
  AudioCacheService._internal() : _database = AudioCacheDatabase(), _dio = Dio(), _config = const AudioCacheConfig() {
    _setupDio();
  }
    ...
    try {
      ...
      if (await file.exists()) {
        ...
        if (actualSize != songFile.fileInfo.size) {
          await file.delete();
          throw Exception(
            'Rozmiar pliku nie zgadza się (oczekiwano: ${songFile.fileInfo.size}, otrzymano: $actualSize)',
          );
        }
        ...
        if (cachedFile != null) {
          await _database.insertOrUpdate(cachedFile.copyWith(checksum: checksum, status: CacheStatus.cached));
        }
