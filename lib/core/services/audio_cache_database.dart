...
class AudioCacheDatabase {
  ...
  Future<int> getTotalCacheSize() async {
    ...
    final result = await db.rawQuery(
      'SELECT SUM(size) as total_size FROM $_tableName WHERE status = ?',
  ...
  Future<Map<String, dynamic>> getCacheStats() async {
    ...
    final playsResult = await db.rawQuery(
      'SELECT SUM(play_count) as total_plays FROM $_tableName WHERE status = ?',
      [CacheStatus.cached.name],
    );
